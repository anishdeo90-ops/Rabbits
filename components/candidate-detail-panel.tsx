                              ...
                              {responses.length > 0 && (
                                <button
                                  onClick={() => {
                                    ...
                                    const body = nonEmpty.length > 0 ? nonEmpty.map(g =>
                                      `<div class="grp">${g.title ? `<div class="sec-hdr">${esc(g.title)}</div>` : ""}${g.rows.map(r => `<div class="row"><span class="lbl">${esc(r.label)}</span><span class="val">${esc(r.value)}</span></div>`).join("")}</div>`
                                    ).join("") : `<div class="empty">No answers in this submission.</div>`;
                                    const submittedDate = new Date(latest.submitted_at).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" });
                                    const printedDate = new Date().toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" });
                                    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${esc(f.name)} - ${esc(cand?.name ?? "")}</title><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:Arial,sans-serif;font-size:12px;color:#1a1a1a;background:#fff}.page{width:210mm;min-height:297mm;margin:0 auto;padding:18mm;background:#fff}.hdr{border-bottom:2.5px solid #ff2d87;padding-bottom:14px;margin-bottom:22px}.hdr h1{font-size:19px;line-height:1.25;font-weight:700;color:#111;margin-bottom:8px}.hdr-meta{font-size:10.5px;color:#555;display:flex;gap:22px;flex-wrap:wrap;align-items:center}.hdr-meta strong{color:#111;font-weight:600}.grp{margin-bottom:18px;page-break-inside:avoid}.sec-hdr{font-size:8.5px;font-weight:700;text-transform:uppercase;letter-spacing:.09em;color:#e91773;border-bottom:1px solid #ffe0ee;padding-bottom:5px;margin-bottom:9px}.row{display:flex;padding:4px 0;border-bottom:1px solid #f3f4f6;align-items:baseline;line-height:1.5;page-break-inside:avoid}.lbl{font-weight:600;color:#555;width:180px;min-width:180px;flex-shrink:0;font-size:11px;padding-right:14px}.val{color:#111;font-size:11.5px;flex:1;word-break:break-word;white-space:pre-wrap}.empty{font-size:12px;color:#777;border:1px dashed #d1d5db;padding:12px}.ftr{margin-top:30px;border-top:1px solid #e5e7eb;padding-top:8px;display:flex;justify-content:space-between;font-size:9px;color:#999}@media screen{body{background:#d1d5db}.page{box-shadow:0 2px 16px rgba(0,0,0,.18);margin:24px auto}}@media print{html,body{background:#fff!important}.page{width:100%;min-height:auto;margin:0;padding:12mm;box-shadow:none}.ftr{position:fixed;bottom:8mm;left:12mm;right:12mm}}</style></head><body><div class="page"><div class="hdr"><h1>${esc(f.name)}</h1><div class="hdr-meta"><span>Candidate:&nbsp;<strong>${esc(cand?.name ?? "-")}</strong></span>${cand?.mobile ? `<span>Mobile:&nbsp;<strong>${esc(cand.mobile)}</strong></span>` : ""}<span>Submitted:&nbsp;<strong>${submittedDate}</strong></span></div></div>${body}<div class="ftr"><span>Hire Rabbits ATS</span><span>Printed&nbsp;${printedDate}</span></div></div><script>window.onload=function(){window.print()}<\/script></body></html>`;
