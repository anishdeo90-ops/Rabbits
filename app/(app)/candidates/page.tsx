...
export default async function CandidatesPage({ searchParams }: { searchParams: Promise<{ status?: string; hr_id?: string; designation_id?: string; designation_name?: string; job_id?: string; owner?: string; site_id?: string; source_id?: string; tag_id?: string; date_from?: string; date_to?: string; date_field?: string; activity_scope?: string; pipeline_stage?: string; forward_to_id?: string }> }) {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect("/login");
  const params = await searchParams;

  const [profileRow, sitesRows, designationsRows, sourcesRows, statusesRows, tagsRows, recruitersRows, interviewersRows] = await Promise.all([
    ...
    supabase.from("masters").select("*").eq("type", "tag").eq("is_active", true).order("sort_order"),
